...
def analyze_gravitational_wave_data():
    ...
    # 执行傅里叶变换
    print("正在进行傅里叶变换...")
    freqs, psd = perform_fft_analysis(h_plus_data, dt)
    ...
    if np.sum(fit_mask) > 10:  # 确保有足够的数据点进行拟合
        ...
        fit_psd = 10**(slope * np.log10(fit_freqs) + intercept)
        ...
        ax5.set_title(f'Power Law Fit ({fit_freq_range[0]}-{fit_freq_range[1]} Hz)')
        ax5.legend()
        ax5.grid(True, alpha=0.3)
    else:
        ax5.text(0.5, 0.5, 'Insufficient data for power law fit', 
                horizontalalignment='center', verticalalignment='center',
                transform=ax5.transAxes)
        ax5.set_title('Power Law Fit')
    
    # 6. 线性坐标下的0.409-0.800Hz拟合图
    ax6 = plt.subplot(2, 3, 6)
    if np.sum(fit_mask) > 10:
        # 在线性坐标下绘制
        ax6.scatter(np.log10(freqs[fit_mask]), np.log10(psd[fit_mask]), s=15, alpha=0.7, color='blue', label='Data')
        ax6.plot(np.log10(fit_freqs), np.log10(fit_psd), 'r-', linewidth=2, 
                label=f'Linear Fit\nSlope: {slope:.3f}\nR²: {r_squared:.3f}')
        
        ax6.set_xlabel('log₁₀(Frequency)')
        ax6.set_ylabel('log₁₀(Power Spectral Density)')
        ax6.set_title('Log-Log Space Linear Fit')
        ax6.legend()
        ax6.grid(True, alpha=0.3)
    else:
        ax6.text(0.5, 0.5, 'Insufficient data for linear fit', 
                horizontalalignment='center', verticalalignment='center',
                transform=ax6.transAxes)
        ax6.set_title('Linear Fit')
    ...
    if np.sum(fit_mask) > 10:
        ...
        print(f"幂律指数 (slope): {slope:.3f}")
        print(f"拟合优度 (R²): {r_squared:.3f}")
        print(f"拟合公式: PSD(f) = 10^({intercept:.3f}) * f^{slope:.3f}")
    else:
        print("数据不足，无法进行拟合")
